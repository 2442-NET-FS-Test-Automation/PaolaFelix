...
// And at the very bottom that app object that represents our entire API calls its run methos

//Builder area
var builder = WebApplication.CreateBuilder(args);

// The first thing that we need is to give our builder a connection string to our database
var conn_string = "Server=localhost,1433;Database=LibraryMinimalDb;User Id=sa;Password=Password1234;TrustServerCertificate=true";
...
// By registering our DbContext class (or even clases, technically you suse one per Database)
// we hand off the managing of creating and destroying these DbContext objects to ASP.NETs
// dependency injection container. Like spring beans if youre familiar.

//ASP.NET has few different scope types
// Transient - a new instance is created every time its requested
// Scoped - a new instance per HTTP request
// Singleton - a single intance for the entire runtime of the app
builder.Services.AddDbContext<LibraryDbContext>(options => options.UseSqlServer(conn_string),
    ServiceLifetime.Scoped, ServiceLifetime.Singleton); // Scoped is the default, but we can be explicit - and allow for SingletonScopes
                                                        // when needed

// we know we will nedd more than one LibraryDbContext in one or more of these methods. But we dont know how man
...
// The logic actually flows from SQL DQL - You can use method OR sql qury syntax
...
app.MapGet("/peek/tracking", (LibraryDbContext db) =>
{
    ...
    // When we create a new object and call the dbsets add method its state is 
    // "added" - this has not actually hit the database yet. But its tracked to be added.
    db.Products.Add(new Product {Sku = "BK-TMP", Name = "Tmp", Price = 1m});

    // This bit of code is the non-production demo bit
    // We are accessing the LibraryDbContext objects change tracker to pull info
    // At most youd debug this.
    var states = db.ChangeTracker.Entries()
        .Select(e => new {entity = e.Entity.GetType().Name, state = e.State.ToString()})
        .ToList();

    // Clearing rhe change tracker manually
    db.ChangeTracker.Clear();

    return states;
});

// Lets manually go out of our way to create a conflict - obviously, dont do this in a real app
app.MapGet("/peek/conflict", (IServiceScopeFactory scopes) =>
{
    ...
    // Now, remmeber that a dbContect is generated per scope, so we have to do that too
    ...
    // remember we gave Inventory entities a RowVersion - not jusut a property named RowVersion
    ...
    firstDb.SaveChanges(); // save changes is what persists any creation, deleted or modified objects
    ...
    // before EF tries to persist any changes, it will chheck the RowVersion. It wont match
    // and an exception will be thrown
    secondInventory.CurrentStock --;

    try
    {
       secondDb.SaveChanges(); // this should fail as RowVersion dont match 
    }
    catch(DbUpdateConcurrencyException ex)
    {
        ...
        // For the entry that threw the exceptions - grab its current values from the DB
    ...
    // I can send back specific codes via methos like .Ok() with messages inside
    // others indlude Problem(), NotFound(), etc
...
app.MapPost("/inventory/rest", (LibraryDbContext db, ILogger<Program> logger) =>
{
    ...
    // what I want to do is rest the items that I know I stuck into de db
            ...
            CancellationToken ct, IFulfillmentService fSvc) =>
{
    ...
    // Now that we've added the order - we try to fulfill i
...
// IHostAppplicationLifetime - this lets us see events related to the app lifetime
// We are going to use it to make sure we "flush" pending order if the app is aked to stop
    ...
    IServiceScopeFactory scopes, IHostApplicationLifetime Lifetime) =>
{
    ...
    _ = Task.Run( async () => // assigning the task result to a discard runs this as a background task
    {
        try
        {
            ...
            var service = scope.ServiceProvider.GetRequiredService<IFulfillmentService>(); // grab a fulfillments service
            await service.FulfillBurstAsync(ids, appStopping); // use it to call fulfillBurstAsync()
        }
        catch(Exception ex)
        {
            // This task is fire and forget becasuse we arent waiting or storing its result
            // any exceptions would be "swallowed" i.e. they would die eith the task in the background
...
app.MapGet("/verify/no-oversell", (LibraryDbContext db) =>
{
    ...
    var fulfilled = db.FulfillmentEvents.Count(e => e.Type == "Fulfilled"); // count fulfilled orders
...
app.MapPost("/benchmark", async (int n, IFulfillmentService fs, ISeeder seeder, CancellationToken ct) =>
{
    ...
    // First, sequential
    var sw1 = Stopwatch.StartNew(); // start 
...
// if for some reason an expedited order's thread slows down (due to some background procces on the computer or something)
...
// My file always ends with app.Run() - minimal API Controller API
app.Run();
Log.CloseAndFlush();

public record OrderPayLoad(int CustomerId, int ProductId, int Quantity);
