...
SELECT UPPER(LastName) AS LastUpper,
        ...
        DATEDIFF(DAY, JoinedDate, GETDATE()) AS DaysMember --Takes 3 argumnets, the datepart (year difference, day difference, day)
FROM dbo.Member;
...
-- This is an example of an equi-join - we join on on equality comparison
...
-- LEFT JOIN - we want all records from the left table, and marching records from the right
...
-- Use join to combine columns from multiple tables into the output
...
-- scalar subquey: books that have more copies than average
SELECT Title, TotalCopies
FROM dbo.Book
WHERE TotalCopies > (SELECT AVG(TotalCopies) FROM dbo.Book); 

-- IN -subquery: member who currently have a book lent out
-- Notice - we did the opposite earlier with a JOIN, this could also simply be a join
...
-- TRANSACTIONS - All of this runs or NONE of it succeds
-- At its most basic Transaction is just BEGIN and CIMMIT
...
-- READ UNCOMMITTED - Queries and statements can read data rows modified by other transactions
    ...
    -- Can accidentally rsult in dirty (untrue) reads, and phantom reads on records that wont exist
    -- once that other transaction finishes.
-- READ COMMITTED (Default) - Prevents dirty read by requiring that data read by a statement must be 
    -- committed before it is processed.
-- REPEATABLE READ - Places shared locks on all data read by transaction and HOLDS THEM until the entire transaction completes
    -- No other transaction can modify or delete this rows during this time. Prevents some dirty reads,
    -- BUT other transactions can insert new rows that may bleed into your filtering from queries
...
-- This is not a saved precompiled dataset. There is something called and indexed view
-- that is outside of our scope that is precompiled. But a VIEW like the one above, a regular VIEW
-- is just a saved select. The data updates to be current whatever you call upon it.
SELECT * FROM dbo.vw_ActiveLoans ORDER BY DueDate;

GO
-- Increasing in complexity and potential usefulness
-- We have stored procedures - a named prgram in the database. Parameters/arguments, logic,
        ...
        BEGIN TRANSACTION
            ...
            IF (SELECT AvailableCopies FROM dbo.Book WHERE BookId = @BookId) <= 0
                ...
                -- THROW takes 3 arguments
                    ...
                    -- message - same string describing the error
...
-- A stored procedure DOES something. Writes, transactions, returning results sets, etc. Called with EXEC
-- A function (user defined or otherwise) computes a value. Its called INSIDE a query, and itself
-- does not change the data inside the database.
...
AS
BEGIN
    --Declaring a variable in SQL that is scoped in this function
